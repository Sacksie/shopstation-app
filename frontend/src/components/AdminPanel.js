...
// Quick Add Price Modal
const QuickAddPriceModal = ({ product, adminPassword, onSave, onClose }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Failed to add price: ${result.error}`);
      }
    } catch (error) {
      ...
      <div className="bg-white rounded-xl max-w-md w-full">
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-xl font-semibold">Add Price: {product.displayName}</h2>
...
const AdminPanel = ({ onBack, onShowAnalytics }) => {
  ...
  // Handle quick add price
  const handleQuickAddPrice = (product) => {
    setQuickAddProduct(product);
  };

  // Handle delete product
  const handleDeleteProduct = async (productKey, productName) => {
    if (!window.confirm(`Are you sure you want to delete "${productName}"? This action cannot be undone.`)) {
      return;
    }

    try {
      const response = await fetch(`${API_URL}/api/manual/delete-product/${encodeURIComponent(productKey)}`, {
        method: 'DELETE',
        headers: {
          'x-admin-password': adminPassword
        }
      ...
      if (result.success) {
        ...
        setMessage(`✗ Error deleting product: ${result.error}`);
      }
    } catch (error) {
  ...
  // Handle inline product name edit
  const handleProductNameEdit = async (productKey, newName) => {
    try {
      ...
      if (result.success) {
        ...
        setMessage(`✗ Error updating product name: ${result.error}`);
      }
    } catch (error) {
      setMessage(`✗ Error updating product name: ${error.message}`);
    }
  };
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded">Auto-Deploy Active</span>
            </div>
          </div>
        </div>
      </div>
                        ...
                        {getProcessedInventoryData().map((product, index) => (
                          <tr key={product.key} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div>
                                {editingProductName === product.key ? (
                                  <input
                                    type="text"
                                    defaultValue={product.displayName}
                                    className="text-sm font-medium text-gray-900 border border-blue-300 rounded px-2 py-1 focus:ring-2 focus:ring-blue-500"
                                    onBlur={(e) => {
                                      if (e.target.value !== product.displayName) {
                                        handleProductNameEdit(product.key, e.target.value);
                                      } else {
                                        setEditingProductName(null);
                                      }
                                    }}
                                  ...
                                  <div 
                                    ...
                                    onClick={() => setEditingProductName(product.key)}
                                ...
                                <button
                                  onClick={() => handleDeleteProduct(product.key, product.displayName)}
                              ...
                              return (
                                <td key={store} className="px-6 py-4 whitespace-nowrap">
                                  {storeData ? (
                                    ...
                                    <button
                                      onClick={() => handleQuickAddPrice(product)}
                                      className="text-green-600 hover:text-green-800 text-sm font-medium"
                                      title="Add price for this store"
                                    >
                                      + Add Price
                                    </button>
                                  )}
